...
class ItemListCreateView(BaseStoreCreateListAPIView):
    ...
    def get_queryset(self):
        return self.service_class(user=self.request.user).get_items(**{"store__id": self.get_store_from_request().id}).order_by("-created_at")
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer.validated_data["store_id"] = (
            self.get_store_from_request().id
        )
...
class ItemRetrieveUpdateDestroyAPIView(BaseStoreRetrieveUpdateDestroyAPIView):
    service_class = ItemService
